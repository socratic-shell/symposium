...
exports.SyntheticPRProvider = void 0;
const vscode = require("vscode");
const syntheticPRTreeProvider_1 = require("./syntheticPRTreeProvider");
/**
 * Content provider for synthetic diff content
 */
class DialecticDiffContentProvider {
    constructor() {
...
class SyntheticPRProvider {
     ...
     */
    async createSyntheticPR(prData) {
        ...
        vscode.window.showInformationMessage(`Synthetic PR created: ${prData.title} (${prData.files_changed.length} files changed)`);
        console.log(`[SYNTHETIC PR] ${Date.now() - startTime}ms: createSyntheticPR completed`);
    }
    /**
     * Update existing synthetic PR
     */
    async updateSyntheticPR(prData) {
    ...
    async showFileDiff(filePath) {
        ...
        try {
            ...
            await vscode.commands.executeCommand('vscode.diff', originalUri, modifiedUri, `${filePath} (PR Diff)`);
     ...
     */
    async generateOriginalContent(fileChange, currentContent) {
    ...
    async createCommentThread(thread) {
        try {
            ...
            const range = new vscode.Range(Math.max(0, thread.line_number - 1), 0, Math.max(0, thread.line_number - 1), 0);
...
exports.SyntheticPRProvider = SyntheticPRProvider;
//# sourceMappingURL=syntheticPRProvider.js.map