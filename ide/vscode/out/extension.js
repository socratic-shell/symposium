...
exports.deactivate = exports.activate = exports.DaemonClient = void 0;
const vscode = require("vscode");
const net = require("net");
const path = require("path");
const crypto = require("crypto");
const syntheticPRProvider_1 = require("./syntheticPRProvider");
const walkthroughWebview_1 = require("./walkthroughWebview");
const bus_1 = require("./bus");
// 💡: Daemon client for connecting to message bus
class DaemonClient {
    constructor(context, outputChannel, syntheticPRProvider, walkthroughProvider) {
        this.context = context;
        this.outputChannel = outputChannel;
        this.syntheticPRProvider = syntheticPRProvider;
        this.walkthroughProvider = walkthroughProvider;
        this.socket = null;
        this.reconnectTimer = null;
        this.isDisposed = false;
        this.buffer = '';
        this.RECONNECT_INTERVAL_MS = 5000; // 5 seconds
        // Terminal registry: track active shell PIDs with MCP servers
        this.activeTerminals = new Set();
        // Review feedback handling
        this.pendingFeedbackResolvers = new Map();
    }
    start() {
    ...
    async handleIncomingMessage(message) {
        ...
        if (message.type === 'present_walkthrough') {
            try {
                const walkthroughPayload = message.payload;
            ...
            try {
                const symbolPayload = message.payload;
            ...
            try {
                const referencesPayload = message.payload;
            ...
            try {
                const prPayload = message.payload;
    ...
    extractShellPidFromMessage(message) {
        return message.shellPid || null;
    }
    async isMessageForOurWindow(shellPid) {
        try {
     ...
     */
    handleCommentFeedback(comment, filePath, lineNumber) {
    ...
    async handleSpecificAction(action, reviewId, resolver) {
        if (action === 'comment') {
     ...
     */
    async collectUserFeedback(reviewId) {
     ...
     */
    async sendReferenceToActiveShell(key, value) {
        const terminals = vscode.window.terminals;
    ...
    sendStoreReferenceToShell(shellPid, key, value) {
        if (!this.socket || this.socket.destroyed) {
     ...
     */
    async resolveSymbolByName(symbolName) {
        try {
            ...
            const resolvedSymbols = symbols.map(symbol => this.vscodeSymbolToSymbolDef(symbol));
    ...
    vscodeSymbolToSymbolDef(symbol) {
    ...
    vscodeLocationToRange(location) {
        return {
     ...
     */
    async findAllReferences(symbol) {
        try {
            ...
            const locations = await vscode.commands.executeCommand('vscode.executeReferenceProvider', vscode.Uri.file(path.isAbsolute(symbol.definedAt.path)
                ? symbol.definedAt.path
                : path.resolve(workspaceFolder.uri.fsPath, symbol.definedAt.path)), new vscode.Position(symbol.definedAt.start.line - 1, symbol.definedAt.start.column - 1));
...
function activate(context) {
    ...
    const bus = new bus_1.Bus(context, outputChannel);
    ...
    const syntheticPRProvider = new syntheticPRProvider_1.SyntheticPRProvider(context);
    bus.setSyntheticPRProvider(syntheticPRProvider);
    // Create walkthrough webview provider
    const walkthroughProvider = new walkthroughWebview_1.WalkthroughWebviewProvider(context.extensionUri, bus);
    bus.setWalkthroughProvider(walkthroughProvider);
    context.subscriptions.push(vscode.window.registerWebviewViewProvider(walkthroughWebview_1.WalkthroughWebviewProvider.viewType, walkthroughProvider));
    // Register walkthrough comment reply command
    const walkthroughCommentCommand = vscode.commands.registerCommand('dialectic.addWalkthroughComment', (reply) => walkthroughProvider.handleCommentSubmission(reply));
    ...
    syntheticPRProvider.setCommentCallback((comment, filePath, lineNumber) => {
    ...
    const reviewActionCommand = vscode.commands.registerCommand('dialectic.reviewAction', (action) => {
...
// 💡: Set up universal selection detection for interactive code review
function setupSelectionDetection(bus) {
    ...
    const codeActionProvider = vscode.languages.registerCodeActionsProvider('*', // All file types
    {
        provideCodeActions(document, range, context) {
            // Only show when there's a non-empty selection
            if (!range.isEmpty) {
                const action = new vscode.CodeAction('Ask Socratic Shell', vscode.CodeActionKind.QuickFix);
                action.command = {
...
async function findQChatTerminal(bus) {
    ...
    if (aiEnabledTerminals.length > 1) {
        ...
        const lastSelectedPID = context.workspaceState.get('dialectic.lastSelectedTerminalPID');
        outputChannel.appendLine(`Last selected terminal PID: ${lastSelectedPID}`);
        const terminalItems = await Promise.all(aiEnabledTerminals.map(async (terminal) => {
...
async function createCompactSelectionReference(selectedText, filePath, startLine, startColumn, endLine, endColumn, bus) {
    try {
        const relativePath = vscode.workspace.asRelativePath(filePath);
...
function formatSelectionMessageLegacy(selectedText, filePath, startLine, startColumn, endLine, endColumn) {
...
async function logPIDDiscovery(outputChannel) {
...
function findVSCodePID(outputChannel) {
