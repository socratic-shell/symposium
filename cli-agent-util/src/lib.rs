...
impl QCLI {
    pub fn detect() -> Result<Box<Self>> {
        let executable_path = which::which("q")
            .context("Q CLI not found in PATH")?;
...
impl ClaudeCode {
    pub fn detect() -> Result<Box<Self>> {
        let executable_path = which::which("claude")
            .context("Claude Code not found in PATH")?;
...
impl CLIAgent for ClaudeCode {
    ...
    fn install_mcp(&self, mcp: &McpServer) -> Result<bool> {
        ...
        cmd.args([
            "mcp", "add",
            "--name", &mcp.name,
            "--command", &mcp.binary_path.to_string_lossy(),
        ]);
